...
// ContextKeyAwareQAction::ContextKeyAwareQAction(QAction* action, ContextKeyExprPtr precondition, 
//   sptr<IContextKeyService> context_key_service) :
//   action_(action), precondition_(precondition) {
//   context_key_service->onDidChangeContext.connect(sig2_t<void (IContextKeyChangeEvent* e)>::slot_type(
//     [this](IContextKeyChangeEvent* e) {
//       std::set<std::string> keys;
//       for (auto k : precondition_->keys()) {
//         keys.insert(k);
//       }
...
// Menu::Menu(std::string id, QString title, QMenu* qmenu, sptr<IContextKeyService> context_key_service) :
...
// MenuBarService::MenuBarService(QWidget* widget, sptr<IThemeService> theme_service, sptr<IContextKeyService> context_key_service) :
//   MenuBarService(new QMenuBar(widget), theme_service, context_key_service) {
// }

// MenuBarService::MenuBarService(QMenuBar* qmenu_bar, sptr<IThemeService> theme_service, sptr<IContextKeyService> context_key_service) :
//   context_key_service_(context_key_service) {
